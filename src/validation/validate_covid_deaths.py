...
SUITE_PATH = "great_expectations/expectations/covid_deaths_suite.json"
...
def load_data_for_validation(run_id: str) -> pd.DataFrame:
    ...
    try:
        query = f"""
            SELECT RUN_ID, SNAPSHOT_DATE, STATE, COVID_19_DEATHS, TOTAL_DEATHS
...
def run_validation(run_id: str) -> dict:
    ...
    # Explicit, in-memory context -- no filesystem scanning.
    ...
    return {
        ...
        "failure_details": [
            {"expectation": name, "result": r.result} for name, r in failed
        ],
    }
...
if __name__ == "__main__":
    ...
    if not result["success"]:
        sys.exit(1)  # non-zero exit lets Airflow/CI treat this as a failed step